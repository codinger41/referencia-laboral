...
{foreach item=job from=$jobs name=tmp}
	<div id="item{$job.id}" class="row settings-row p15" style="padding: 10px;">
		<div class="icons" >
			...
			{/if}
			<a href="{$BASE_URL_ADMIN}edit-post/{$job.id}/" title="Edit this job"><i class="fa fa-gear fa-lg mr10" aria-hidden="true"></i></a>
		...
		<span class="f14"><span class="light">at</span> <span class="moderate-black" style="color: #39b6b3">{$job.company}{if $job.is_location_anywhere}, {$translations.jobs.location_anywhere}{else} </span><span class="light">in</span> <span class="moderate-black">{$job.location}</span>{/if}</span>
		<div>
			<span class="fs13 gray">viewed <strong>{$job.views_count} times</span></strong>{if $statisticalData[$job.id]}, <strong>{$statisticalData[$job.id].numberOfApplications} applicants</strong>, 
